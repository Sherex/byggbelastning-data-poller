...
module.exports.setupDB = async () => {
  ...
  const createClientsLocationTable = `
    CREATE TABLE IF NOT EXISTS clients_location (
      ...
      UNIQUE(time, location, building, floor)
    );
  `
  ...
  const createClientsCoordinatesTable = `
    CREATE TABLE IF NOT EXISTS clients_coordinates (
      ...
      UNIQUE (time, uid, mapHierarchyString, x, y)
    );
  `
  ...
  const createLocationsTable = `
    CREATE TABLE IF NOT EXISTS locations (
      mapHierarchyString   TEXT           NOT NULL UNIQUE,
      campus               TEXT           NOT NULL,
      building             TEXT           NOT NULL,
      floor                TEXT           NOT NULL,
      imageBase64          TEXT
...
module.exports.insertClientCoords = async (data) => {
  ...
  try {
    const query = format(`
      INSERT INTO clients_coordinates (time, uid, mapHierarchyString, campus, building, floor, x, y)
        VALUES %L
...
module.exports.insertLocationMaps = async (data) => {
  ...
  try {
    const query = format(`
      INSERT INTO locations (mapHierarchyString, campus, building, floor, imageBase64)
        VALUES %L
