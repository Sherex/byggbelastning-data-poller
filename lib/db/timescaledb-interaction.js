...
module.exports.insertClientCoords = async (data) => {
  ...
  const values = data.map(client => {
    ...
    return [new Date(client.eventTime), uid, client.floorRefId, client.x, client.y]
  })

  try {
    const query = format('INSERT INTO clients_coordinates(time, uid, floorRefId, x, y) VALUES %L', values)
